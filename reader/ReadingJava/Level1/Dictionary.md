...
Dictionaries are useful data structures - think of a list of words, each one with a definition (and sometimes many more than one). We were all taught to use them very early in our educational careers, and there is a surprising amount of computer science that references such a powerful data structure.

A `dictionary` is a powerful metaphor for an extremely useful data structure in most computing languages.
