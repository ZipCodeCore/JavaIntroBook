...
A Java programmer writes code which in _classes_ and then, when a java program 
runs, _objects_ are created. 
...
It doesn't contain the Airplane object itself, but it knows where to find 
it in memory. If you assign 

```
Airplane plane2 = plane1;
```
 
both references point to the same object and changing the object 
through one reference affects the other.
...
Is is also the source of confusion, if you happen to make a mistake and that's NOT what you hadd in mind,
you may have to untangle it a bit.
