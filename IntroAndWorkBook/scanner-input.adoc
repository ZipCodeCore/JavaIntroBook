...
The `Scanner` object connects to `System.in`, which represents the keyboard input.
Always remember to close the scanner when you're done with it using `scanner.close()`.
...
2. **Be careful with mixing `nextInt()` and `nextLine()`**. After reading a number, you might need to call `scanner.nextLine()` to consume the remaining newline character.
...
4. Prints a personalized message using all three pieces of information
