...
A void method in Java is a method that does not return any 
value at the end of the computation. 
The keyword void is used in the method declaration to 
indicate this. Instead of sending data back, a void method 
typically performs an action, 
such as printing output or modifying an object.
...
The simplest way to pass information to a method is through a 
single parameter. This allows you to create flexible methods 
that can work with different values each time they're called, 
making your code more reusable and powerful.
...
When your method needs more than one piece of information to do 
its job, you can pass multiple parameters separated by commas. 
This is essential for methods that perform calculations or 
operations involving several values, like mathematical functions 
or data processing.
...
Methods can accept parameters of different data types in the 
same parameter list. This flexibility allows you to create 
methods that work with various kinds of data simultaneously, 
such as combining text, numbers, and boolean values to perform 
complex operations.
...
One way to think about programming is to think about how the data of
program moves around.
A programmer controls, and manipulates, the data that is input into 
the program, and passes it around as various computations are done.

Having methods handle certain computations, and then produce results, well, you need a way to 
_return_ those results.
...
Making decisions based on data is a central point of programming.
You need to do different things based on different data, so it is very
common to have If statements in methods that use conditions to perform
computations.
...
Let's create a `Student` class that demonstrates various 
types of methods:
...
Validation methods are essential for ensuring data integrity 
and making logical decisions in your programs. 
These methods typically return boolean values (true or false) 
and are used to check conditions, validate input, or determine 
program flow. They're commonly used in if statements and loops.
...
Utility methods are general-purpose functions that perform 
common operations you might need throughout your program. 
These methods encapsulate frequently-used calculations or 
conversions, making your code more organized and eliminating 
the need to write the same logic multiple times.
